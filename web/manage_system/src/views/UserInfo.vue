  ...
  <CardContentVue>
    ...
    <tbody>
      <tr v-for="user in users" :key="user.id">
        ...
        <!-- <div>{{ $store.state.user.id }} + {{ user.id }}</div> -->
        <td v-if="parseInt($store.state.user.id) === user.id">
          <button data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo" type="button" class="btn btn-primary" style="margin-right: 10px;">修改</button>
          <button type="button" class="btn btn-danger">删除</button>
        </td>
        <td v-else>
          <!-- <button type="button" class="btn btn-danger">没有操作权限</button> -->
        </td>
      </tr>
    </tbody>
</table>
